...
1. **每日总结 (DailySummary)**: 提供日记记录、明日计划功能，作为用户每日复盘的入口。新增“补录”功能，支持手动添加遗漏的专注记录。
2. **任务管理 (TaskManager)**: 支持任务的创建、分类（学习、生活、习惯）、状态流转（待办、进行中、已完成）及拖拽排序。
3. **专注时钟 (FocusTimer)**: 提供番茄钟或自定义计时功能，记录专注时长与标签。支持通过统一的补录组件手动添加记录。
...
- **单元测试**: 已引入 Vitest，针对关键组件（如 `DailySummary` 的补录按钮）和工具函数进行了单元测试。
- **集成/E2E 测试**: 已添加针对核心流程（如补录功能）的端到端测试 (`supplementary.e2e.ts`)。
- **Linting**: 配置了 ESLint 进行代码规范检查。

**建议**:

- 继续扩大单元测试覆盖率，特别是针对复杂的 Hooks 和 Service 层逻辑。
- 完善 E2E 测试场景，覆盖任务管理和数据统计等核心模块。
